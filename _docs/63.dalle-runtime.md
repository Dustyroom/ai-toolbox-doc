...
The best way to get started with the ChatGPT API is to open the `[Demo] ChatGPT` scene in the `Assets/AiToolbox/Samples/Runtime Usage/[Demo] ChatGPT` folder. This scene demonstrates how to use the `ChatGpt` class to send requests to the ChatGPT API and receive the response.

![](../assets/images/manual_images/runtime-sample-screenshot.png){: .image-fancy }

{:.image-caption}
The `[Demo] ChatGPT` scene
