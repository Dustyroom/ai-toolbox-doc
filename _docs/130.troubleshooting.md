...
> - You may want to try using a **Use Proxy Server** option of AI Toolkit. Please check [this paragraph](/getting-started/#openai-api-key) for more details.
...
> - **Re-import {{ site.title_short }}**. Right-click on Assets/AI Toolbox folder and select **Reimport**. This will re-import all the files from the package. You also may want to delete the **AI Toolbox** folder from the Assets folder and re-import the package again. Sometimes cleaning the cache before re-importing the package from the Package Manager may help. The cache can be found here:
...
> - Check if you have the latest version of AI Toolbox. You can check it in the Package Manager.
