...
**New!** **Anthropic Claude** is now available in {{ site.title_short }}! You can now use Claude in Unity, one of the best tools for coding. Check out the [Claude](/claude-set-up/) page for more details.
{:.notice--success}
...
{{ site.title_short }} also provides runtime features for using various AI models in your shipped games. Runtime usage is useful for generating dynamic content, such as NPC dialogue, procedural generation, and more.
