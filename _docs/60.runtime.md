...
These examples demonstrate how to send a request to ChatGPT using `QuickRequest` and `StreamRequest` methods.
...
Additionally, the error code could be a value of HTTP status code, e.g. 401, 429, 500, etc. You can find the full list of HTTP status codes returned by the ChatGPT API [here](https://platform.openai.com/docs/guides/error-codes).
...
The ChatGPT API can return different HTTP status codes. You can find the full list of HTTP status codes returned by the ChatGPT API [here](https://platform.openai.com/docs/guides/error-codes).
