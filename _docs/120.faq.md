...
- We've added an option to use a free API server. Although it's become more unstable lately and using proxy has its limits, you can try your luck with it. Please check out a **Use Proxy Server** option in the {{ site.title_short }} [settings](/getting-started/#openai-api-key) for instructions.
