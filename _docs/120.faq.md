...
The [Claude](/claude-set-up) models from Anthropic are also a great option, with pricing similar to OpenAI's.
...
#### Is {{ site.title_short }} aware of scripts, assets, settings in my project?
No, the AI services used in {{ site.title_short }} are not aware of your project files, scripts, or assets. They generate code based solely on the prompts you provide. However, you can include relevant information in your prompts to guide the AI in generating code that fits your project's context.
You can use the ["Chat with AI"](/chat/) window (Window ▶︎ {{ site.title_short }}) to ask how to use a specific framework or asset. If there is a particular script you know AI should use, you can attach it as context in the chat window.

---

#### Why is "..." AI service not supported?
We are constantly working on adding support for more AI services. However, some services may not have (yet) a public API that would allow us to integrate them into Unity. If you have a particular service in mind, please [get it touch](/contact-details/) and let us know.
