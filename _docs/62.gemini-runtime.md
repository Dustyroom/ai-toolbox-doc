---
title: "Gemini Runtime Usage"
permalink: /gemini-runtime/
toc: true
---

![](../assets/images/manual_images/runtime-icon-dialogue.svg){: .image-simple }

Runtime Gemini usage is a new feature introduced in **{{site.title_short}} 6.3.0**. It allows you to use the Gemini API in your Unity project at runtime. This feature is useful for creating interactive applications, games, and other projects that require real-time communication with the Gemini API.
{:.notice--success}

## Overview

The `Gemini` class provides a simple and convenient way to interact with the [Gemini API](https://ai.google.dev/gemini-api/docs/models/gemini){:target="_blank"}. It allows you to send requests to the Gemini API and receive the response either all at once or in chunks as it is generated.

## Sample Scene

The best way to get started with the Gemini API is to open the `[Demo] Gemini` scene in the `Assets/AiToolbox/Samples/Runtime Usage/[Demo] Gemini` folder. This scene demonstrates how to use the `Gemini` class to send requests to the Gemini API and receive the response.

![](../assets/images/manual_images/runtime-sample-screenshot.png){: .image-fancy }

{:.image-caption}
The `[Demo] Gemini` scene
...
Sends a request to Gemini and receives the response. The response is provided in the `completeCallback` function. If the request fails, the `failureCallback` function is called.
...
- `completeCallback`: The function to be called on successful completion. Gemini response is provided as a parameter.
...
Represents the settings for the AI Toolbox Gemini requests.
...
- `apiKey`: The API key for Gemini. This property is required. You can find your API key on the [API Keys](https://platform.openai.com/account/api-keys){:target="_blank"} page.
...
These examples demonstrate how to send a request to Gemini using the `Request` method.
...
The following code example demonstrates how to send request to Gemini and receive the response. The example uses the `Parameters` class to set the API key and other parameters from the Unity Inspector.
...
void Start() {
    ...
    Gemini.Request(prompt, parameters,
                    response => {
                        Debug.Log("Full response: " + response);
                    }, (errorCode, errorMessage) => {
                        var errorType = (GeminiErrorCodes)errorCode;
...
If you need to facilitate a dialogue between the user and the AI, you can use the `Request` method with a sequence of `Message` objects. The following code example demonstrates how to use the `Gemini` class to send a request to Gemini and receive the response. The example uses the `Parameters` class to set the API key and other parameters from the Unity Inspector.

Please see the [Sample Scene](#sample-scene) for a complete example.

## Error Handling

When using the `AiToolbox.Gemini` class, errors can occur during requests. Errors are handled through the `failureCallback` parameter of the `Request` method. This callback provides an error code and an error message as parameters.

The error codes are represented by the `GeminiErrorCodes` enum:
...
Additionally, the error code could be a value of HTTP status code, e.g. 401, 429, 500, etc. You can find the full list of HTTP status codes returned by the Gemini API [here](https://platform.openai.com/docs/guides/error-codes){:target="_blank"}.
...
Gemini.Request(
    ...
    (errorCode, errorMessage) => {
        GeminiErrorCodes error = (GeminiErrorCodes)errorCode;
        Debug.LogError("Error: " + errorCode + " - " + errorMessage);

        switch (error) {
            case GeminiErrorCodes.MaxTokensExceeded:
                // Handle max tokens exceeded error
                break;
            case GeminiErrorCodes.ThrottleExceeded:
                // Handle throttle exceeded error
                break;
            case GeminiErrorCodes.RemoteConfigConnectionFailure:
                // Handle remote config connection failure error
                break;
            case GeminiErrorCodes.RemoteConfigKeyNotFound:
                // Handle remote config key not found error
                break;
            case GeminiErrorCodes.Unknown:
...
The Gemini API can return different HTTP status codes. You can find the full list of HTTP status codes returned by the Gemini API [here](https://platform.openai.com/docs/guides/error-codes){:target="_blank"}.

## API Key Management

Proper management of the Gemini API key is essential to keep your account secure and prevent unauthorized access. AI Toolbox provides different ways to store the API key in a Unity project, and you should choose the method that best suits your security needs.

### Storing the API Key in a GameObject

You can store the Gemini API key directly in a GameObject by making the [`Parameters`](#parameters-class) a public property. This allows you to set the API key directly in the Unity Inspector. However, if you use the Encryption Option `None` the API key will be stored in plain text, which can expose it to malicious actors.
...
If you have any questions or need help with the Gemini API, please [contact us](/contact-details/).
