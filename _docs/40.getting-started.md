...
- **Advanced Options** — if expanded, reveals the [_Temperature_](/getting-started/#temperature), [_Timeout_](/getting-started/#timeout) and [_History_](/getting-started/#prompt-history) parameters, mirrored from the [_Settings panel_](/getting-started/#chatgpt-script-generator-settings) (which can be found in _Edit_ ▶︎ _Project Settings_ ▶︎ _ChatGPT Script Generator_).
...
The maximum time in seconds to waut for a response from ChatGPT. If ChatGPT doesn't respond within the _Timeout_, the script generation fails. Current default _Timeout_ is 60 seconds.
