...
- **Open File** - This will open the generated shader file in the default application for the file type. Shortcut: `Ctrl/Cmd + Enter/Return`.
...
![Edit Shader with ChatGPT button](/assets/images/manual_images/gpt-shader-edit-button.png)
</a>
...
With {{ site.title_short }}, you can chat with ChatGPT within Unity. You can ask ChatGPT to explain the code, ask it how to do operations in Unity etc. If ChatGPT outputs any piece of code, you have an option to save it as a file in the project.

To open the ChatGPT window, go to _Window_ ▶︎ _{{ site.title_short }}_ ▶︎ _Chat with ChatGPT_.
...
{:.image-caption}
Chat with ChatGPT menu item

Once you open the chat window, you are good to ask away. You'll see the _Send a message_ field where you can type your message in the bottom of the window. Type your prompt and press the send button (shortcut: `Ctrl/Cmd + Enter/Return`).

- **'Pencil' button** - This will open the prompt window where you can edit the prompt to make another generation attempt.
- **Copy button** - Copy the ChatGPT response to clipboard.
- **Save button** - Saves the code within the ChatGPT response as a file in the project.
- **Regenerate response** - Tries generating another variation of the reply from ChatGPT to your latest prompt. If the Temperature is set to 0 in the _Settings_ foldout menu (described below), this will generate the same response.
...
- **'Paper airplane' button** - Send the message. Shortcut: `Ctrl/Cmd + Enter/Return`.

An obvious thing, but just to remind, the results of the chatting with ChatGPT, including the code explanations, how-to's etc largely depend on the [GPT model](/getting-started/#gpt-model) set in the AI Toolbox settings.
{:.notice--info} 
...
- **Use Proxy Server** - Use the proxy URL for the OpenAI API.
