...
- **Chatbot** - the menu for choosing the Backend for generating the scripts. Currently, there are two options: _OpenAI ChatGPT_, _Azure ChatGPT_ and _Google Gemini_. The default chatbot is _OpenAI ChatGPT_. The Backend used for [Chat with AI](/#chat-with-ai) can be changed in _Chat with AI_ window's Settings.
...
- **Settings** - Expands and collapses the foldout menu revealing the settings applicable to the chat.
  ...
  - **Chatbot** - Select the chatbot to use. Currently, there are two options: _ChatGPT_ and _Google Gemini. The default chatbot is _ChatGPT_. **This selects the chatbot for the chat only** — the chatbot used for other generation purposes (scripts, images etc) can be changed in the [AI Toolbox Settings](/getting-started/#ai-toolbox-settings).
...
- **GPT model** (on the window footer) - This indicates which [chatbot and model](/getting-started/#gpt-model) is being used for the generation — for the responses within this freeform chat only. Currently, the models are  **GPT-3.5**, **GPT-4** and **Google Gemini**. The model used for other generation purposes (scripts, images etc) can be changed in the [AI Toolbox Settings](/getting-started/#ai-toolbox-settings).
