...
- **GPT model** (on the window footer) - This is the button that upon click switches between [GPT models](/getting-started/#gpt-model) for responses just for the chat. Currently, these models are available: **GPT3.5**, **GPT4**, **Google Bard**. The model used for other generation purposes (scripts, images etc) can be changed in the [AI Toolbox Settings](/getting-started/#ai-toolbox-settings).
...
The _Chat with ChatGPT_ window is dockable and can be placed anywhere in the Unity Editor.
{:.notice--info}
