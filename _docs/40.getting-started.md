...
- **GPT version** (on the window footer) - This is the button that upon click switches between [GPT models](/getting-started/#gpt-model) for responses just for the chat. The model used for other generation purposes (scripts, images etc) can be changed in the [Ai Toolbox Settings](/getting-started/#ai-toolbox-settings).
