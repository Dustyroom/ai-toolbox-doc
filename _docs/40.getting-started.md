...
- **GPT model** (on the window footer) - This is the button that upon click switches between installed [GPT models](/getting-started/#gpt-model) for responses just for the chat. Currently, the models are  **GPT-3.5**, **GPT-4** and **Google Bard**. The model used for other generation purposes (scripts, images etc) can be changed in the [AI Toolbox Settings](/getting-started/#ai-toolbox-settings).
